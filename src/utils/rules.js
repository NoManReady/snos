...
export const maskValidator = (rule, mask, cb) => {
  ...
  rule.message = rule.message || '请输入有效的掩码地址'
...
export const dnsValidator = (rule, dns, cb) => {
  if (dns === '') {
    ...
    rule.message = rule.message || '请输入有效的DNS地址，多个以空格隔开'
    let _max = rule.max || 5
    let _dns = dns.replace(/^\s*|\s*$/g, '').replace(/\s+/g, ' ').split(/\s+/)
    if (_dns.length > _max) {
...
export const intValidator = (rule, val, cb) => {
  ...
  rule.message = rule.message || '请输入正整数'
  validate(_valid, rule, cb)
}
...
export const macValidator = (rule, val, cb) => {
  ...
  rule.message = rule.message || 'MAC格式 00:11:22:33:44:55'
  validate(_valid, rule, cb)
}
...
export const chineseValidator = (rule, val, cb) => {
  ...
  rule.message =
    rule.message || '名称不可包含中文' + (rule.flag ? '及中文字符' : '')
  validate(!_valid, rule, cb)
}
...
export const ssidNameValidator = (rule, val, cb) => {
  ...
  rule.message =
    rule.message ||
    `Wi-Fi名称不能超过${_size}(有空格${_size - 2})个字符(单个中文占3个字符)`
  validate(_valid, rule, cb)
}
...
export const rangeHexValidator = (rule, value, cb) => {
  if (value === '') {
    ...
    let _valid = utils.isBetween(parseInt(value, 16), parseInt(rule.min, 16), parseInt(rule.max, 16))
