...
function getIpStartAndIpEnd(ipRange) {
  ...
  if (typeof ipRange === 'object') {
    ...
    let _maskLong = ipToLong(ipRange.netmask)
...
export const isExistIpRange = (ip, index, list) => {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (_curIpRange.ipEnd < _targetIpRange.ipStart || _curIpRange.ipStart > _targetIpRange.ipEnd) {
      continue
    }
    return true
  }
...
export const ipNetValidate = (r, v, cb) => {
  if (v && !isNetSeg(v))
    return cb(new Error('子网格式：192.168.110.0/24'))
  cb()
}