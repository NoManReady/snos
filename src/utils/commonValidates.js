...
function getIpStartAndIpEnd(ipRange) {
  const _getRange = (ipLong = '', maskLong = '', is32Mask = false) => {
    if (is32Mask) {
      // 32位掩码特殊处理
      return {
...
export const judgeIpRangeRules = (list, index) => {
  const isExistIp = (r, v, cb) => {
    if (isExistIpRange(v, index, list)) {
      return cb(new Error(I18N.t('rules.exist_ip_range_or_addr')));
    }
    cb();
  };
  const isLegalIp = (r, v, cb) => {
    if (!isIp(v) && !isIpRange(v)) {
      return cb(new Error(I18N.t('rules.invalid_ip_range_or_addr')));
    }
    cb();
  };
  return [
    { required: true, message: I18N.t('rules.empty_ip_range_or_addr') },
    { validator: isLegalIp },
    { validator: isExistIp }
  ];
};
...
export const isIpInNet = (ip, netList = []) => {
  ...
  let isBelong = _ipRangeList.some(item => {
    return (
      _curIpRange.ipStart >= item.ipStart && _curIpRange.ipEnd <= item.ipEnd
    );
  });
  return isBelong;
};
...
export const ipNetValidate = (r, v, cb) => {
  if (v && !isNetSeg(v)) return cb(new Error(I18N.t('rules.net_example')));
  let _ip = v.split('/')
  let ip = longToIp(ipToLong(_ip[0]) & netCodeToLong(_ip[1]))
  if (_ip[0] !== ip)
    return cb(new Error(I18N.t('rules.ip_mask_diffrent_network')))
  cb();
};
