...
export const ipRegexp = /^((25[0-5]|2[0-4]\d|1\d\d|[1-9]\d|\d)\.){3}(25[0-5]|2[0-4]\d|1\d\d|[1-9]\d|\d)$/;

export const ipRangeRegxp = /^((25[0-5]|2[0-4]\d|1\d\d|[1-9]\d|\d)\.){3}(25[0-5]|2[0-4]\d|1\d\d|[1-9]\d|\d)\s*-\s*((25[0-5]|2[0-4]\d|1\d\d|[1-9]\d|\d)\.){3}(25[0-5]|2[0-4]\d|1\d\d|[1-9]\d|\d)$/;

export const portRangeRegxp = /^([1-9][0-9]{0,3}|[1-5][0-9]{4}|6[0-4][0-9]{3}|65[0-4][0-9]{2}|655[0-2][0-9]{1}|6553[0-5])\s*-\s*([1-9][0-9]{0,3}|[1-5][0-9]{4}|6[0-4][0-9]{3}|65[0-4][0-9]{2}|655[0-2][0-9]{1}|6553[0-5])$/;
...
// 。 ？ ！ ， 、 ； ： “ ” ‘ ' （ ） 《 》 〈 〉 【 】 『 』 「 」 ﹃ ﹄ 〔 〕 … — ～ ﹏ ￥
export const chineseTagRegexp = /[\u3002|\uff1f|\uff01|\uff0c|\u3001|\uff1b|\uff1a|\u201c|\u201d|\u2018|\u2019|\uff08|\uff09|\u300a|\u300b|\u3008|\u3009|\u3010|\u3011|\u300e|\u300f|\u300c|\u300d|\ufe43|\ufe44|\u3014|\u3015|\u2026|\u2014|\uff5e|\ufe4f|\uffe5]/;
...
export const netSegment = /^((25[0-5]|2[0-4]\d|1\d\d|[1-9]\d|\d)\.){3}(25[0-5]|2[0-4]\d|1\d\d|[1-9]\d|\d)\/([1-9]|[1-2][0-9]|30|31|32)$/;

export const urlRegexp = /^((https|http|ftp|rtsp|mms):\/\/)?(([0-9a-z_!~*'().&=+$%-]+: )?[0-9a-z_!~*'().&=+$%-]+@)?((([1-9]?[0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]).){3}([1-9]?[0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])|([0-9a-z_!~*'()-]+.)*([0-9a-z][0-9a-z-]{0,61})?[0-9a-z].[a-z]{2,6})(:([1-9]?[0-9]|[1-9][0-9]{2}|[1-9][0-9]{3}|[1-5][0-9]{4}|6[0-5][0-5][0-3][0-5]))?((\/?)|(\/[\0-9a-zA-Z_!~*'().;?:@&=+$,%#-]+)+\/?)$/;

// 不包含http头的网址（域名） 或 IP
export const domainRegexp = /^(([0-9a-z_!~*'().&=+$%-]+: )?[0-9a-z_!~*'().&=+$%-]+@)?((([1-9]?[0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]).){3}([1-9]?[0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])|([0-9a-z_!~*'()-]+.)*([0-9a-z][0-9a-z-]{0,61})?[0-9a-z].[a-z]{2,6})(:([1-9]?[0-9]|[1-9][0-9]{2}|[1-9][0-9]{3}|[1-5][0-9]{4}|6[0-5][0-5][0-3][0-5]))?((\/?)|(\/[\0-9a-zA-Z_!~*'().;?:@&=+$,%#-]+)+\/?)$/;
...
export const getMaskByIp = ip => {
  ...
  let mask;
  if (!isIp(ip)) {
    err(I18N.t('rules.invalid_ip_format'));
  }
  for (let i = 3; i >= 0; i--) {
    if (ipArr[i] == 0) {
      zeroNum--;
...
export const isMacMask = mask => {
  ...
  return (
    _isMac && /^1+0*$/.test(parseInt(mask.split(':').join(''), 16).toString(2))
  );
};
...
export const vlanValidate = (val, m) => {
  let _message = I18N.t('rules.vlan_range_s');
  let _vlans = [];
  if (!val) {
    return I18N.t('rules.require_vlanid');
  }
  if (m) {
    ...
    if (!_allValid) {
      return I18N.t('rules.vlan_err_m');
    }
    _splits.forEach(sp => {
    ...
    if (!/^[1-9]\d*$/.test(val)) {
      return I18N.t('rules.digit_err_format');
    }
    _vlans = [val];
  }
