...
function pushRequest(config) {
  ...
  if (!LOADING) {
    LOADING = Loading.service({
      lock: true,
      text: config.text || I18N.t('fetch.loading'),
...
(function (w) {
  ...
  w.BASE_URI =
    w.BASE_URI ||
    location.pathname.split(/\/;stok/)[0] ||
...
export default (url, data = {}, setting = {}) => {
  if (!_errMap['esw']) {
    _errMap['esw'] = errorMap.esw();
  }
  if ((window.APP_ROLES || []).includes('msw') && !_errMap['msw']) {
    _errMap['msw'] = errorMap.msw();
  }
  ...
  const _promise = new Promise((resolve, reject) => {
    ...
    let _random = {
      ...
      text:
        setting.text ||
        (['set', 'add', 'del', 'update'].includes(data.method.split('.')[1])
          ? I18N.t('action.editing')
          : null)
    };
      ...
      .then(res => {
        ...
        if (responseData.data) {
          try {
            ...
            } else if (data && typeof responseData.data === 'string') {
              responseData.data = JSON.parse(responseData.data);
            }
          } catch (error) {
            log('Returns a non JSON string', responseData.data);
          }
        }
        setting.noParse && log(responseData)
        // rpc获取data
        ...
        if (_responseData && _responseData['dev_type'] === 'esw') {
          ...
          if (_code !== 0) {
            responseData.error = {
              message: _errMap['esw'][_code] || _responseData.result
            };
          } else {
      ...
      .catch(res => {
        ...
        if (!_response) {
          if (!res.isCancel) {
            try {
              if (res.message.indexOf('timeout') > -1) {
                _message = I18N.t('fetch.req_timeout');
              } else {
                _message = I18N.t('fetch.req_err_check');
              }
            } catch (e) { }
          } else {
          ...
          switch (_response && _response.status) {
            case 404:
              _message = I18N.t('fetch.req_err_404');
              break;
            case 403:
            case 401:
              saveToSession(
              ...
              _message = I18N.t('fetch.req_forbid');
              break;
            case 408:
              _message = I18N.t('fetch.req_timeout');
              break;
            case 500:
              _message = I18N.t('fetch.req_err_inner');
              break;
            case 501:
              _message = I18N.t('fetch.function_no_ok');
              break;
            case 503:
              _message = I18N.t('fetch.server_is_nouse');
              break;
            case 504:
              _message = I18N.t('fetch.gateway_err');
              break;
            default:
              _message = _response.message || I18N.t('fetch.unknow_err');
          }
        }
        if (_dev && _message) {
          // 错误提示
