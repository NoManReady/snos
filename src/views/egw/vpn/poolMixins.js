import { ipRangeRegxp, ipToLong, getLowAddr, getHighAddr } from "@/utils/rulesUtils";
...
export default {
  ...
  computed: {
    ...
    lanIpRangeList() {
      return this.lans.map(o => getLowAddr(o.ipaddr, o.netmask) + "-" + getHighAddr(o.ipaddr, o.netmask));
    }
  },
  async created() {
    this.getNetwork()
    ...
    getPoolRules(isAddPool = false, checkIpRange = true) {
      ...
      const isLegalIp = (r, v, cb) => {
        ...
        if (checkIpRange) {
          ...
          if (isExistIpRange(v, this.editIndex, this.ipRangeList) || isExistIpRange(v, this.editIndex, this.lanIpRangeList)) {
