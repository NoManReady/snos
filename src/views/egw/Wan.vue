<template>
  ...
  >
    ...
    <template v-if="baseModel.proto==='pppoe'">
      ...
      <p class="steps-wrap c-info" v-if="!isRemote">
        <i :class="learnOk ? 'el-icon-success c-success': 'el-icon-info'"></i>
        <label>
          {{learnOk ? "已获取到账号密码。再次" : "忘记账号密码？"}}
          <a @click="_onShowPppoeSteps" class="c-success pointer">从旧设备中获取账号密码</a>
        </label>
      </p>
    </template>
    <template v-else>
...
export default {
  ...
  watch: {
    ...
    'baseModel.password'(v) {
      v === '' && (this.learnOk = false)
    }
  },
  methods: {
    _getModelData() {
    ...
    _onShowPppoeSteps() {
      Steps()
      bus.$on('pppoe-learn-callback', res => {
