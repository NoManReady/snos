<template>
  <div class="box component-overview-ifaceinfo">
    <div class="box-header">
      <span class="box-header-tit">{{ $t("overview.if_info") }}</span>
    </div>
    <div
      class="component-overview-ifaceinfo-port tc"
      v-show="Object.keys(portStatus).length"
    >
      ...
      <div class="vm mt15 ml10" v-for="(ports, k) in portStatus" :key="k">
            ...
            <div slot="content">
              <p>
                {{ $t("network.speed_f")
                  ...
                  p.speed === "auto" ? $t("phrase.auto") : (p.speed || 0) + "M"
                }}
              </p>
              <template v-if="p.poe_direct !== 'none'">
                <p>
                  POE{{ $t("symbol.semic")
                    ...
                    p.poe_direct === "in"
                      ? $t("network.receive_power")
                      : $t("network.supply_power")
                  }})
                </p>
              </template>
            </div>
        ...
        <p class="tc component-overview-ifaceinfo-port--desc">
          {{ k.toUpperCase() == "NULL" ? "&nbsp;" : k }}
        </p>
      </div>
    </div>
  </div>
