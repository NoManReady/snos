  ...
  <div class="switch-vlansetting" v-loading="isLoading">
    <el-form :model="baseModel" :rules="baseRules" class="mb10" label-width="80px" ref="baseForm" size="mini">
      <el-row :gutter="0">
        ...
        <el-col :span="12">
          <el-form-item label="类型" prop="vlanType">
        ...
        <el-col :span="12">
          <el-form-item label="Native" prop="native">
            <el-select class="w100" v-model="baseModel.native">
              <el-option :key="vlan .vid" :label="`VLAN ${vlan .vid}`" :value="vlan .vid" v-for="vlan in vlanMembers"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
      </el-row>
    ...
    >
      ...
      <el-table-column align="center" label="VLAN类型" width="100">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      baseRules: {
        port: [{ required: true, message: '请选择端口号' }],
        permit: [{ validator: permitValidator }],
        native: [{ required: true, message: '请选择Native VLAN' }]
      },
      isLoading: false
    }
  },
  ...
  computed: {
    ...mapGetters('switch', ['item', 'portInfoList', 'portTreeList']),
    vlanTreeList() {
        ...
        if (valid) {
          let _postData = {
            port: this.baseModel.port.includes('all')
              ? Array.from({ length: this.portInfoList.length }).map(
                  (_, index) => index
                )
              : [...this.baseModel.port],
