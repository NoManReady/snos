...
import { wirelessFn, healthyFn } from '@/model/modules/wireless'
...
export default {
  ...
  computed: {
    ...
    // 独立模式/从模式不加载分组模块
    hideGroup() {
      return this.$roles().includes('alone') || this.$roles().includes('slave')
    },
    // wifi名称列表
    ssidNames() {
  ...
  methods: {
    ...
    async _loadWifi(loading) {
      ...
      // 检测插入访客Wi-Fi
      // let _vistorSsid = _result.ssidList.find(ssid => ssid.wlanId === '8')
      // if (!_vistorSsid) {
      //   _result.ssidList.push({
      //     ...wirelessFn(),
      //     ssidName: this.defaultVistorName, // 默认访客Wi-Fi名称
      //     enable: 'false',
      //     guest: 'true',
      //     vlanId: '1',
      //     wlanId: '8'
      ...
      // }
      // 和MACC一致，根据wlanId排序
      _result.ssidList.sort(
        (s1, s2) => s1.wlanId - s2.wlanId
      )

      // 从设备根据能力值显示应用频段
      if (this.$roles().includes("slave")) {
        _result.ssidList = _result.ssidList.filter(o =>
          // 过滤掉不支持的频段WiFi
          this.rsMap.find(ro => o.relatedRadio.includes(ro.v))
        ).map(o => Object.assign(o, {
          // 只显示支持的应用频段
          relatedRadio: this.rsMap.length === 1 ? this.rsMap[0].v : o.relatedRadio
        }))
      }
    ...
    formatVlanId(row, column) {
      if (this.specialVlans.includes(row.vlanId)) {
        return this.isRouter ? '默认VLAN' : '与AP同VLAN'
      } else {
        return row.vlanId
      }
    },
    ...
    _updateSsidList(ssid) {
      ...
      let _idx = this.wireless.ssidList.findIndex(
        s => s.wlanId === ssid.wlanId
      )
      if (_idx != -1) {
        this.wireless.ssidList.splice(_idx, 1, ssid)
      } else {
        this.wireless.ssidList.unshift(ssid)
      }
    ...
    _postWifiData(ssid = false) {
      ...
      let _loading = null
      if (_isInit) {
        _loading = Loading.service({ text: '正在创建网络' })
      }
      let _data = deepClone(this.wireless)
      _data.ssidList = _data.ssidList.filter(ssid => !!ssid.ssidName)
      return this.$api.setWireless(_data).then(() => {
        if (_isInit) {
          this._loadWifi(false).then(() => {
            _loading.close()
          })
        } else {
          this.$message({
